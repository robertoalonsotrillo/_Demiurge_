...
1. The prediction database is processed into *descriptor input sequences* and saved in `descriptor database II`. The **descriptor model** predicts subsequent descriptor strings based on that data.

2. The audio database is processed into `descriptor database I` and each descriptor linked to an `ID reference` connected to the specific audio segment.
